...
const ProfilePage = () => {
   ...
   /*  First time I see the page I should see an empty disabled form (doesn't look like a form), with only the labels being visible.
    ...
    When I click on the save button (which is a submit button) and the new info is sent to the backend,
    if there's an error keep the form enabled.
    If not, the form is back to being disabled
    and after that the new data is displayed on the profile. */
